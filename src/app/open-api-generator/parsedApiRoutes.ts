...
export class ParsedApiRoutes {
  ...
  private reservedTypes = new Set<string>([
    "void",
    "any",
    "string",
    "Record",
  ...
  public parse(route: any) {
    ...
    const allParams = this.parseParams(
      route.routeParams.query,
      ParamType.QUERY,
    );
    ...
    logger.info(
      `parsing route: ${route.request.method?.toUpperCase()} ${route.raw.route}`,
    );
    const apiRoute: ApiRoute = {
      ...
      allParams: allParams,
      shouldWrapReturnResultInJSON: this.shouldWrapReturnResultInJSON(
        route.response,
      ),
      shouldAllowRelaxedTypes: this.shouldAllowRelaxedTypes(
        route,
        allParams,
        this.sanitizeTypes(route.response.type),
      ),

      isQuery: route.raw.method === "get",

      successResponseIsVoid: this.successResponseIsVoid(route.response),
      successAndErrorResponseIsVoid: this.successAndErrorResponseIsVoid(
        route.response,
      ),
    };
  ...
  private shouldWrapReturnResultInJSON(response: any): boolean {
    if (response["type"] === "any" || response["type"] === "void") {
      ...
      response["type"].includes("Record<") ||
      response["type"].includes("Map<")
    ) {
      return true;
    }
    return false;
  ...
  private successAndErrorResponseIsVoid(response: any): boolean {
    return (
      response.errorType === "void" && this.successResponseIsVoid(response)
    );
  }

  private shouldAllowRelaxedTypes(
    apiRoute: any,
    allParams: Param[] | undefined,
    ...
    let sanitizedType = responseSuccessType.endsWith("[]")
  ...
  private addTypeToImportList(type: string, importList: Set<string>) {
    ...
    for (type of allTypes) {
      ...
      if (
        !this.reservedTypes.has(this.getImportType(type)) &&
        this.generatedComponents.has(type)
      ) {
  ...
  private sortParamsByOptionality(params: Param[]) {
    // puts all required params at the start of the array and the optional ones at the end
  ...
  private parseParams(args: any[], paramType: ParamType): Param[] {
    // TODO: change parameter name to `routeParametersArray` or something more descriptive
  ...
  private getTypeMapping(routeParam: any): string {
    ...
    return tsTypeMapping.get(routeParam["type"])
      ? tsTypeMapping.get(routeParam["type"])
      : routeParam["type"];
  }
}
