...
export function getParsedSchema(typeNames: generatorTypes.GeneratedTypeName[], schemaComponents: swaggerTypescriptApi.SchemaComponent[]): SchemaParser {
  const schemaParser = new SchemaParser();

  for (const typeName of typeNames) {
    schemaParser.createTypeNameMapping(typeName.typeName, typeName.rawTypeName!);
  }
  ...
  Array.from(schemaParser.mappings.refToSchemaMap.values()).forEach((schema) => {
...
class SchemaParser {
  ...
  parseSchemaProperty(schemaProperty: parserTypes.SchemaProperty, schema: parserTypes.Schema, visitedRefs: Set<string>) {
    ...
    if (parserTypes.schemaPropertyIsRelaxedType(schemaProperty)) {
      ...
      const requiresRelaxedTypeJsDocTag = this.parseSchema(newSchema, visitedRefs);
